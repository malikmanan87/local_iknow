        ...
        <div class="card-panel">
            <div class="card-panel-header py-3">
                <h5 class="card-panel-title">
                    <i class="bi bi-gear-fill me-2 text-primary"></i>Tetapan Sistem (System Settings)
                </h5>
            </div>
                ...
                <form action="<?= base_url('settings/update') ?>" method="post" id="settingsForm">
                    ...
                    <ul class="nav nav-tabs mb-4" id="settingsTab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link small fw-semibold py-2" id="security-tab" data-bs-toggle="tab" data-bs-target="#security" type="button" role="tab">
                                <i class="bi bi-shield-check me-1"></i> Keselamatan & Sistem
                            </button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content" id="settingsTabContent">
                        ...
                        <div class="tab-pane fade show active" id="general" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label required">Nama Aplikasi (App Name)</label>
                                    <input type="text" name="app_name" class="form-control" 
                                           value="<?= old('app_name', $settings['app_name'] ?? ($sysSettings['app_name'] ?? APP_NAME)) ?>" 
                                           placeholder="Cth: Sistem Saya" required>
                                    <div class="form-text small text-muted">Akan dipaparkan pada tajuk halaman, sidebar, dan e-mel sistem.</div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label required">Slogan Aplikasi (App Tagline)</label>
                                    <input type="text" name="app_tagline" class="form-control" 
                                           value="<?= old('app_tagline', $settings['app_tagline'] ?? ($sysSettings['app_tagline'] ?? '')) ?>" 
                                           placeholder="Masukkan deskripsi pendek..." required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Organisasi / Syarikat</label>
                                    <input type="text" name="company_name" class="form-control" 
                                           value="<?= old('company_name', $settings['company_name'] ?? ($sysSettings['company_name'] ?? '')) ?>" 
                                           placeholder="Nama pemilik sistem...">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Zon Masa (Timezone)</label>
                        ...
                        <div class="tab-pane fade" id="email" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label required">E-mel Penghantar (System Email)</label>
                                    <input type="email" name="system_email" class="form-control" 
                                           ...
                                           placeholder="Cth: noreply@domain.com" required>
                                    <div class="form-text small text-muted">Digunakan sebagai alamat emel penghantar notifikasi automatik.</div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Protokol Penghantaran</label>
                                    ...
                                    <select name="email_protocol" class="form-select">
                                        ...
                                        <option value="smtp" <?= $proto === 'smtp' ? 'selected' : '' ?>>SMTP (Disyorkan untuk pengeluaran)</option>
                                    </select>
                                </div>
                                <div class="col-md-8">
                                    <label class="form-label">Hos SMTP (SMTP Host)</label>
                                    <input type="text" name="smtp_host" class="form-control" 
                                           value="<?= old('smtp_host', $settings['smtp_host'] ?? ($sysSettings['smtp_host'] ?? '')) ?>" 
                                           placeholder="Cth: smtp.googlemail.com">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Port SMTP</label>
                                    <input type="number" name="smtp_port" class="form-control" 
                                           value="<?= old('smtp_port', $settings['smtp_port'] ?? ($sysSettings['smtp_port'] ?? '')) ?>" 
                                           placeholder="Cth: 465 atau 587">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="security" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label">Mod Penyelenggaraan (Maintenance Mode)</label>
                                    <div class="form-check form-switch mt-2">
                                        ...
                                        <label class="form-check-label text-secondary" for="maintenanceSwitch">Aktifkan Mod Penyelenggaraan</label>
                                    </div>
                                    <div class="form-text small text-danger mt-1">Apabila diaktifkan, pengguna biasa tidak boleh mengakses sistem kecuali peranan Admin.</div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Had Percubaan Log Masuk (Login Throttling)</label>
                                    ...
                                    <select name="login_attempts" class="form-select">
                                        <option value="3" <?= $attempts === '3' ? 'selected' : '' ?>>3 Kali Percubaan</option>
                                        <option value="5" <?= $attempts === '5' ? 'selected' : '' ?>>5 Kali Percubaan (Standard)</option>
                                        <option value="10" <?= $attempts === '10' ? 'selected' : '' ?>>10 Kali Percubaan</option>
                                    </select>
                                    <div class="form-text small text-muted">Akaun akan dikunci sementara jika salah kata laluan melebihi had.</div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Tempoh Sesi (Session Lifespan)</label>
