...
export default function AddFlowModal({ moduleId, nextStepNumber, onClose, onSuccess, submodules = [] }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Gagal menambah flow: ' + (err.response?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Tambah Langkah Aliran Kerja (Flow)</h3>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            {formData.image_path && (
              <div style={{ marginTop: '0.5rem', fontSize: '0.8rem', color: 'var(--accent-emerald)' }}>
                ✓ Imej berjaya dimuat naik: {formData.image_path}
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.5rem' }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading || uploading}>
              <Save size={16} /> {loading ? 'Menyimpan...' : 'Simpan Flow'}
            </button>
          </div>
        </form>
      </div>
